...
def update_policy(net, target_net, optimizer, memory, batch_size, gamma, device):
...
def main():
    ...
    policy_net = DQN(in_channels=6, n_actions=n_actions).to(device)
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            if random.random() < epsilon:
                ...
                with torch.no_grad():
                    combined_state_tensor = torch.FloatTensor(combined_state).unsqueeze(0).to(device)
            ...
            update_policy(policy_net, target_net, optimizer, memory, batch_size, gamma, device)
        # plt.close()
        # env.close()
